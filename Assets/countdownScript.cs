...
public class countdownScript : MonoBehaviour
{
    ...
    private int[] largeNumbers = new int[4] { 25, 50, 75, 100 };
    ...
    void Update()
    {
        if(!moduleSolved && mostRecentSolve == target)
        {
            ...
            moduleSolved = true;
            Debug.LogFormat("[Countdown #{0}] You have made {1}. Module solved.", moduleId, target);
    ...
    void Start()
    {
        ...
        while ((target < 100 || target > 999))
        {
            ...
            Debug.LogFormat("<Countdown #{0}> chosenEqIndex = {1}, solutionTest[chosenEqIndex] = {2}, target = {3}", moduleId,chosenEqIndex, solutionTest[chosenEqIndex], target);
            ...
            if (target < 100 || target > 999)
            {
                ...
                Debug.LogFormat("<Countdown #{0}> solutionTest = {1}", moduleId, string.Join(", ", outArray2));
            }
        }
        Logging();
    }
    ...
    void GenerateNumbers()
    {
        ...
        {
            int index3 = UnityEngine.Random.Range(1,11);
    ...
    void Logging()
    {
        ...
        if(chosenEquation == 0)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} × {2}) + ({3} - {4}) + ({5} - {6}).", moduleId, selectedNumbers[5], selectedNumbers[1], selectedNumbers[0], selectedNumbers[4], selectedNumbers[2], selectedNumbers[3]);
        }
        else if(chosenEquation == 1)
        {
            if(selectedNumbers[2] > selectedNumbers[3])
            {
                Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} - {2} + {3}) × ({4} - {5}).", moduleId, selectedNumbers[0], selectedNumbers[1], selectedNumbers[5], selectedNumbers[2], selectedNumbers[3]);
            }
            else if(selectedNumbers[3] > selectedNumbers[2])
            {
                Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} - {2} + {3}) × ({4} - {5}).", moduleId, selectedNumbers[0], selectedNumbers[1], selectedNumbers[5], selectedNumbers[3], selectedNumbers[2]);
            }
            else
            {
                Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} - {2} + {3}) × ({4} + {5}).", moduleId, selectedNumbers[0], selectedNumbers[1], selectedNumbers[5], selectedNumbers[3], selectedNumbers[2]);
        ...
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} + {2} + {3}) - {4}) + ({5} × {6}).", moduleId, selectedNumbers[0], selectedNumbers[4], selectedNumbers[2], selectedNumbers[1], selectedNumbers[3], selectedNumbers[5]);
        }
        else if(chosenEquation == 3)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} x {2}) + ({3} × {4}) - ({5} + {6}).", moduleId, selectedNumbers[5], selectedNumbers[1], selectedNumbers[4], selectedNumbers[3], selectedNumbers[0], selectedNumbers[2]);
        }
        else if(chosenEquation == 4)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} - {2} - {3}) × {4}) + {5} - {6}.", moduleId, selectedNumbers[1], selectedNumbers[3], selectedNumbers[4], selectedNumbers[5], selectedNumbers[0], selectedNumbers[2]);
        }
        else if(chosenEquation == 5)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} + {2}) - ({3} + {4}) + ({5} × {6}).", moduleId, selectedNumbers[1], selectedNumbers[0], selectedNumbers[2], selectedNumbers[3], selectedNumbers[4], selectedNumbers[5]);
        }
        else if(chosenEquation == 6)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} × {2}) + {3} - {4} - ({5} + {6}).", moduleId, selectedNumbers[0], selectedNumbers[4], selectedNumbers[1], selectedNumbers[5], selectedNumbers[2], selectedNumbers[3]);
        }
        else if(chosenEquation == 7)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} + {2}) × ({3} - {4}) + {5} - {6}.", moduleId, selectedNumbers[5], selectedNumbers[4], selectedNumbers[2], selectedNumbers[3], selectedNumbers[0], selectedNumbers[1]);
        }
        else if(chosenEquation == 8)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} × ({2} + {3})) - {4}) + {5}.", moduleId, selectedNumbers[2], selectedNumbers[4], selectedNumbers[5], selectedNumbers[3], selectedNumbers[0]);
        }
        else if(chosenEquation == 9)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} + {2} - {3}) × {4}) + {5} - {6}.", moduleId, selectedNumbers[1], selectedNumbers[5], selectedNumbers[2], selectedNumbers[0], selectedNumbers[4], selectedNumbers[3]);
        }
        else if(chosenEquation == 10)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} - {2}) × ({3} + {4}).", moduleId, selectedNumbers[2], selectedNumbers[3], selectedNumbers[4], selectedNumbers[5]);
        }
        else if(chosenEquation == 11)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} × {2}) + {3} + {4}.", moduleId, selectedNumbers[0], selectedNumbers[5], selectedNumbers[4], selectedNumbers[3]);
        }
        else if(chosenEquation == 12)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: ({1} × {2} × {3}) - ({4} + {5}).", moduleId, selectedNumbers[5], selectedNumbers[4], selectedNumbers[3], selectedNumbers[0], selectedNumbers[2]);
        }
        else if(chosenEquation == 13)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} + {2}) × {3}) - {4}.", moduleId, selectedNumbers[1], selectedNumbers[5], selectedNumbers[4], selectedNumbers[3]);
        }
        else if(chosenEquation == 14)
        {
            Debug.LogFormat("[Countdown #{0}] One possible solution: (({1} - {2}) + ({3} - {4})) × {5}.", moduleId, selectedNumbers[1], selectedNumbers[4], selectedNumbers[0], selectedNumbers[2], selectedNumbers[5]);
        }
    }
    ...
    void executeOperation()
    {
        ...
        if(selectedOperation == "+")
        {
            ...
            {
                Debug.LogFormat("[Countdown #{0}] Strike! {1} + {2} would yield a number greater than 9,999.", moduleId, firstPress.chosenNumber, secondPress.chosenNumber);
        ...
        else if(selectedOperation == "−")
        {
            ...
            {
                Debug.LogFormat("[Countdown #{0}] Strike! {1} - {2} would yield a negative number.", moduleId, firstPress.chosenNumber, secondPress.chosenNumber);
                GetComponent<KMBombModule>().HandleStrike();
                Reset();
                return;
            }
        ...
        else if(selectedOperation == "÷")
        {
            ...
            {
                Debug.LogFormat("[Countdown #{0}] Strike! {1} ÷ {2} would yield a non-integer.", moduleId, firstPress.chosenNumber, secondPress.chosenNumber);
                GetComponent<KMBombModule>().HandleStrike();
                Reset();
                return;
            }
    ...
    public void StartClock()
    {
        if(!clockOn)
        {
            ...
            clockOn = true;
            Debug.LogFormat("[Countdown #{0}] Here's the countdown clock...", moduleId);
        }
        else
        {
            return;
    ...
    IEnumerator clockRoutine()
    {
        ...
        if(!moduleSolved)
        {
            Debug.LogFormat("[Countdown #{0}] Strike! You have run out of time.", moduleId);
            GetComponent<KMBombModule>().HandleStrike();
    ...
    public IEnumerator ProcessTwitchCommand(string command)
    {
        ...
        foreach (string cmd in cmds)
        {
            if (Regex.IsMatch(cmd, @"^\s*(?:press|select)?\s*(?:activate|go|start|clock)\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
            {
                ...
                Debug.LogFormat("[Countdown #{0}] TP Command: Your next line is... \"Here's the countdown clock...\"", moduleId);
            ...
            else if ((mt = Regex.Match(cmd, @"^\s*(?:press|select)?\s*(\d{1,4})\s*([+\-*×x/÷])\s*(\d{1,4})\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)).Success)
            {
                ...
                {
                    Debug.LogFormat("[Countdown #{0}] TP Command: {1} plus {2}", moduleId, numberA, numberB);
                    operSel = operators[0];
                }
                else if (operatorTx.Equals("-"))
                {
                    Debug.LogFormat("[Countdown #{0}] TP Command: {1} minus {2}", moduleId, numberA, numberB);
                    operSel = operators[1];
                }
                else if (operatorTx.Equals("/") || operatorTx.Equals("÷"))
                {
                    Debug.LogFormat("[Countdown #{0}] TP Command: {1} divided by {2}", moduleId, numberA, numberB);
                    operSel = operators[3];
                }
                else // X, x, *, ×
                {
                    Debug.LogFormat("[Countdown #{0}] TP Command: {1} times {2}", moduleId, numberA, numberB);
                    operSel = operators[2];
                }
